...
class FormFiller {
  constructor(gen, validators, settings = {}, skipSelectors = [], customFields = []) {
    this.gen = gen;
  ...
  /**
   * Look up a stored custom-value entry for a field (matched by elementId or
   * fieldName, scoped to the current page title, same as _shouldSkip)
   */
  _getCustomFieldEntry(field) {
    if (!this.customFields || this.customFields.length === 0) return null;

    const fieldName = field.name;
    const fieldId = field.id;
    const currentPageTitle = document.title;

    return this.customFields.find(entry => {
      if (entry.elementId && fieldId && entry.elementId === fieldId && entry.pageTitle === currentPageTitle) {
        return true;
      }
      if (entry.fieldName && fieldName && entry.fieldName === fieldName && entry.pageTitle === currentPageTitle) {
  ...
  async generateValue(field) {
    ...
    // Custom user-supplied values take priority over detection/dv
    const customEntry = this._getCustomFieldEntry(field.element);
    if (customEntry && customEntry.values && customEntry.values.length > 0) {
      return customEntry.values[Math.floor(Math.random() * customEntry.values.length)];
    }
  ...
  async fillForm(form) {
    ...
    // Handle radio/checkbox groups with custom user-supplied option values
    // (these are excluded from the generic text-fill path above)
    const handledRadioGroups = new Set();
    for (const field of fields) {
      if (field.fieldType !== 'radio' && field.fieldType !== 'checkbox') continue;

      const entry = this._getCustomFieldEntry(field.element);
      if (!entry || !entry.optionValues || entry.optionValues.length === 0) continue;

      if (field.fieldType === 'radio') {
        const groupKey = field.element.name || field.element.id;
        if (handledRadioGroups.has(groupKey)) continue;
        handledRadioGroups.add(groupKey);

        const groupElements = field.element.name
          ? Array.from(form.querySelectorAll(`input[type="radio"][name="${CSS.escape(field.element.name)}"]`))
          : [field.element];

        const choice = entry.optionValues[Math.floor(Math.random() * entry.optionValues.length)];
        groupElements.forEach(el => {
          el.checked = (el.value === choice);
          el.dispatchEvent(new Event('change', { bubbles: true }));
        });
      } else {
        // Checkbox: each eligible option independently has a 50% chance of being checked
        const key = field.element.value || 'checked';
        const eligible = entry.optionValues.includes(key);
        field.element.checked = eligible ? Math.random() < 0.5 : false;
        field.element.dispatchEvent(new Event('change', { bubbles: true }));
      }
    }
